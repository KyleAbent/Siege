...
function Commander:Logout()
    ...
    --Handle Coloration difference of Lights between Commanders and Normal players
    local powerPoint = GetPowerPointForLocation(self:GetLocationName())
    if powerPoint then
--McG: FIXME This only slightly mitigates the problems, need a "full-update" forced call on any current BaseLightWorker
    --Force-run lights to change asap
        powerPoint.lastUpdatedTime = nil
    end
end
...
local function UpdateSentryBatteryLine(self, fromPoint)
    ...
    local PowerConsumersNearBy = GetEntitiesWithMixinForTeamWithinRange("PowerConsumer", self:GetTeamNumber(), fromPoint, BackupBattery.kRange)
    Shared.SortEntitiesByDistance(fromPoint, PowerConsumersNearBy)

    local closestSentry = PowerConsumersNearBy[1]
    ...
    if closestSentry then
        ...
        if distance and distance <= BackupBattery.kRange then
            inRange = true
        end
