...
Shared.RegisterNetworkMessage("DebugTraceVisualization",
{
    startPoint = "vector",
    endPoint = "vector",
    fraction = "float",
...
local traces = {}
if Client then
    local function DebugVisTraceHelper(startPoint, endPoint, fraction, lifetime)
        local newTraceViz = {}
    ...
    Client.HookNetworkMessage("DebugTraceVisualization", function(msg)
        DebugVisTraceHelper(msg.startPoint, msg.endPoint, msg.fraction, msg.lifetime)
    end)
    function Debug_VisualizeTrace(startPoint, trace, lifetime)
        DebugVisTraceHelper(startPoint, trace.endPoint, trace.fraction, lifetime)
    end
end
...
if Server then
    function Debug_VisualizeTrace(startPoint, trace, lifetime)
        ...
        assert(type(startPoint) == "cdata")
        assert(startPoint:isa("Vector"))
    
        assert(type(endPoint) == "cdata")
        assert(endPoint:isa("Vector"))
    
        assert(type(fraction) == "number")
        
        Server.SendNetworkMessage("DebugTraceVisualization",
        {
...
if Predict then
    function Debug_VisualizeTrace(trace, lifetime)
    end
end
