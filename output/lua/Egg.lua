...
function GetLifeformEggAllowed(techId, origin)
    ...
    if locationName then
        ...
        for _, egg in ientitylist(Shared.GetEntitiesWithClassname("Egg")) do
            ...
            if egg:GetLocationName() == locationName and egg:GetGestateTechId() ~= kTechId.Skulk then
                table.insert(lifeformEggs, egg)
            end

        end

        return #lifeformEggs < kUpgradedEggsPerHive, "COMMANDERERROR_TOO_MANY_UPGRADED_EGGS"
...
local function GestatePlayer(self, player, fromTechId)
    ...
    newPlayer:SetIsDroppedEmbryo(true) -- Not player evolve, just players "Using" a lifeform egg.
...
function Egg:SetQueuedPlayerId(playerId)
    ...
    playerToSpawn:GetTeam():RemovePlayerFromRespawnQueue(playerToSpawn)
