...
function Embryo:GetGestationSoftCappedAmount(amount)
    ...
    -- will be nil if spawning this in with console commands.
    self.gestationStartTime = self.gestationStartTime or Shared.GetTime()
...
local function UpdateGestation(self)
    ...
    if self:GetIsAlive() and self.gestationClass ~= nil then
        ...
        if self.evolveTime >= self.gestationTime then
            ...
            newPlayer:UpdateArmorAmount(0, 0)
...
function Embryo:OnInitialized()
    ...
    if not Predict then
        InitMixin(self, EggVariantMixin)
        self:ForceEggSkinsUpdate()
    end
...
function Embryo:GetBaseCarapaceArmorBuff()
    return 0
end

function Embryo:GetCarapaceBonusPerBiomass()
    return 0
end
...
if Server then
    ...
    function Embryo:SetIsDroppedEmbryo(isDroppedEmbryo)
        self.isDroppedEmbryo = isDroppedEmbryo
    end

    function Embryo:GetIsDroppedEmbryo()
        return self.isDroppedEmbryo
    end
