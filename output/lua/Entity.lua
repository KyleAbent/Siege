...
local Shared_GetEntitiesWithTagInRange = Shared_GetEntitiesWithTagInRange

local tarray = table.array
local tempResults = tarray(16)

function Shared.GetEntitiesWithTagInRange(tag, origin, radius, filter, results)
    -- Create the table with collocated array part
    results = results or tarray(3)
    local count

    if filter then
        count = Shared_GetEntitiesWithTagInRange(tag, origin, radius, tempResults)
        ...
        count = Shared_GetEntitiesWithTagInRange(tag, origin, radius, results)
    end
    return results
end
...
local function ientitylist_it(entityList, currentIndex)
    ...
    if currentIndex >= numEntities then
        return nil
    end
    ...
    while currentIndex < numEntities do
        currentEnt = entityList:GetEntityAtIndex(currentIndex)
        currentIndex = currentIndex + 1
...
-- Entity class lists are static over the lifetime of the VM and can be cached once retrieved
local ClassNameCache = {}
local TagCache = {}

local oldGetWithClassname = Shared.GetEntitiesWithClassname
function Shared.GetEntitiesWithClassname(name)
    local entities = ClassNameCache[name]

    if not entities then
        entities = oldGetWithClassname(name)
        ClassNameCache[name] = entities
    end

    return entities
end

local oldGetWithTag = Shared.GetEntitiesWithTag
function Shared.GetEntitiesWithTag(name)
    local entities = TagCache[name]

    if not entities then
        entities = oldGetWithTag(name)
        TagCache[name] = entities
    end
...
function GetEntitiesAliveForTeam(className, teamNumber)
    local teamFilterFunction = CLambda [=[args ent; HasMixin(ent, "Team") and ent:GetTeamNumber() == self[1] and HasMixin(ent, "Live") and ent:GetIsAlive()]=] {teamNumber}
    return GetEntitiesWithFilter(Shared.GetEntitiesWithClassname(className), teamFilterFunction)
end

--Note: this only works with ScriptActor type (Entity does not have innate locationId property)
function GetEntitiesAliveForTeamByLocationWithTechId( className, teamNumber, locationId, techId )

    local filterFunction = CLambda [=[
        (...):isa("ScriptActor") and (...).locationId == self[2]
        and HasMixin(..., "Tech") and (...):GetTechId() == self[3]
        and HasMixin(..., "Team") and (...):GetTeamNumber() == self[1]
        and HasMixin(..., "Live") and (...):GetIsAlive()
	]=] {teamNumber, locationId, techId}
...
--Note: this only works with ScriptActor type (Entity does not have innate locationId property)
function GetEntitiesAliveForTeamByLocation( className, teamNumber, locationId )
...
function GetEntitiesAliveForTeamWithinRange(className, teamNumber, origin, range)

    local TeamFilterFunction = CLambda [=[
        HasMixin(..., "Live") and (...):GetIsAlive()
		and HasMixin(..., "Team") and (...):GetTeamNumber() == self[1]
	]=] {teamNumber}
...
function GetEntitiesWithinXZRange(className, origin, range)

    local inRangeXZFilterFunction = Closure [=[
		self origin range
		args entity
		return (entity:GetOrigin() - origin):GetLengthSquaredXZ() <= (range * range) and not entity:isa("Commander")
	]=] {origin, range}
...
function GetEntityInfo(entity)  --FIXME Needs to handle overflow better (output gets truncated)
