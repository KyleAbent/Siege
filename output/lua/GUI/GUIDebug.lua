...
G_AddDebugCommand = AddDebugCommand
...
local function SetHighlightEnabled(state)
    ...
    if highlightEnabled then
        ...
        highlightItem:SetOptionFlag(GUIItem.CorrectScaling)
    else
...
local kAlign2HotSpot = { [0] = 0, [1] = 0.5, [2] = 1 }
Event.Hook("UpdateClient", function(deltaTime)
    if highlightItem then
        ...
        if hasItem then
            ...
            -- Adjust hot spot of item for text alignment
            local alignX = currentItemTbl.item:GetTextAlignmentX()
            local alignY = currentItemTbl.item:GetTextAlignmentY()
            highlightItem:SetHotSpot(kAlign2HotSpot[alignX], kAlign2HotSpot[alignY])
        end
    end
end)
...
local kValidAlignments = { ["0"] = "Align_Min", ["1"] = "Align_Center", ["2"] = "Align_Max" }
AddDebugCommand("g_set_text_align_x", "Calls SetTextAlignmentX for the current item (not exposed as a property)",
function(alignment)
    
    if alignment == nil or not kValidAlignments[alignment] then
        Log("usage: g_set_text_align_x 0|1|2 (0=min, 1=center, 2=max)")
        return
    end
    ...
    item:SetTextAlignmentX(tonumber(alignment))
    
    Log("Set item text alignment x value to %s", kValidAlignments[alignment])
    
end)

AddDebugCommand("g_set_text_align_y", "Calls SetTextAlignmentY for the current item (not exposed as a property)",
function(alignment)
    
    if alignment == nil or not kValidAlignments[alignment] then
        Log("usage: g_set_text_align_y 0|1|2 (0=min, 1=center, 2=max)")
        return
    end
    ...
    item:SetTextAlignmentY(tonumber(alignment))
    
    Log("Set item text alignment y value to %s", kValidAlignments[alignment])
