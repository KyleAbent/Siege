...
-- Only applied per sender-eventname-receiver set.
-- This also indirectly implies the max number of things that can be added to a layout object in a single frame (OnParentChanged event)
local kMaxEventFiresPerFrame = 5000
local eventFireCountsBySender = {}
...
function GUIEventManager:ProcessEventQueue()
    ...
    while #self.queuedCallbacks[currentLevel] > 0 do
        ...
        if callbackRef then
            ...
            if not callbackRef.removed then
                ...
                local numFiresThisFrame = 1
                if callback.sender and callback.eventName and callback.receiver then

                    -- Make sure table exists
                    if not eventFireCountsBySender[callback.sender] then
                        eventFireCountsBySender[callback.sender] = {}
                        eventFireCountsBySender[callback.sender][callback.eventName] = {}
                    elseif not eventFireCountsBySender[callback.sender][callback.eventName] then
                        eventFireCountsBySender[callback.sender][callback.eventName] = {}
                    end

                    numFiresThisFrame = (eventFireCountsBySender[callback.sender][callback.eventName][callback.receiver] or 0) + 1
                    eventFireCountsBySender[callback.sender][callback.eventName][callback.receiver] = numFiresThisFrame

                end

                --
                --Print(string.format("$ Processing Event - Sender: %s, EventName: %s, Count: %s, Receiver: %s",
                --        ToString(callback.sender),
                --        ToString(callback.eventName),
                --        ToString(numFiresThisFrame),
                --        ToString(callback.receiver)))

                if numFiresThisFrame > kMaxEventFiresPerFrame then

                    error(string.format("Detected infinite GUIObject events! Sender: %s, EventName: %s, Receiver: %s",
                            ToString(callback.sender),
                            ToString(callback.eventName),
                            ToString(callback.receiver))
                    )
