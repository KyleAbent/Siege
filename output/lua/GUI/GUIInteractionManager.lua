...
function GUIInteractionManager:Initialize()
    ...
    -- The object that the mouse was right-clicked down on.
    self.rightClickObj = nil
...
function GUIInteractionManager:OnObjectDestroyed(guiObject)
    ...
    if guiObject == self.rightClickObj then
        self.rightClickObj = nil
    end
...
function GUIInteractionManager:StopListeningForMouseCursor(guiObject, triggeringItem)
    ...
    -- Make sure the object we're no longer listening for isn't the one we right-clicked down on.
    if guiObject == self.rightClickObj then
        self.rightClickObj = nil
    end
...
-- Forward-declare these locals so we can reference them from each other.
local ProcessMouseClickEvent = nil
local ProcessMouseReleaseEvent = nil

local kKey2MouseButtonName =
{
    [InputKey.MouseButton0] = "LMB",
    [InputKey.MouseButton1] = "RMB",
}
ProcessMouseClickEvent = function(self, key)
    
    DBGClickLog("GUIInteractionManager ProcessMouseClickEvent(key=%s)", kKey2MouseButtonName[key])

    local isRightClick = key == InputKey.MouseButton1

    -- Cancel other clicks (eg right clicking while left button is held down).
    if isRightClick and self.clickObj ~= nil then
        local forceCancel = true
        ProcessMouseReleaseEvent(self, InputKey.MouseButton0, forceCancel)
    end

    if not isRightClick and self.rightClickObj ~= nil then
        local forceCancel = true
        ProcessMouseReleaseEvent(self, InputKey.MouseButton1, forceCancel)
    end
    ...
    for i=0, guiItemArray:GetSize() - 1 do
        ...
        if MouseOverItemCheck(self, guiObject, mousePos) then
            ...
            local double = false
            if not isRightClick and
               guiObject == self.doubleClickObj and
               now <= self.doubleClickTime and
               guiObject:GetCanBeDoubleClicked() then

                double = true
            end

            if isRightClick then
                self.rightClickObj = guiObject
            else
                if double then
                    self.doubleClickObj = nil
                    self.clickObj = nil
            ...
            if isRightClick then
                DBGClickLog("Calling OnMouseRightClick() for item '%s'.", bka)
            else
            ...
            if self.tempClickObj then
                if isRightClick then
                    SendOnGUIObjectRightClicked(guiObject)
                else
    ...
    if GetModalObject(self) then
        ...
        if gClickEventLog then
            if isRightClick then
                DBGClickLog("calling OnOutsideRightClick for modal item '%s'.", Debug_GetBKAForItem(modalObj:GetRootItem()))
            else
...
ProcessMouseReleaseEvent = function(self, key, forceCancel)
    
    DBGClickLog("GUIInteractionManager ProcessMouseReleaseEvent()")

    local isRightButton = key == InputKey.MouseButton1

    if not isRightButton and self.clickObj == nil then
    ...
    if isRightButton and self.rightClickObj == nil then

        DBGClickLog("no object was right-clicked down on to consume release event.")
        return false -- no item was clicked down on (that we care about).
    ...
    self.tempObject = isRightButton and self.rightClickObj or self.clickObj
    ...
    if not forceCancel and mouseOverRect and MouseOverItemCheck(self, self.tempObject, mousePos) then
        if isRightButton then
            DBGClickLog("cursor over item, calling OnMouseRightRelease()...")
            self.tempObject:OnMouseRightRelease()
        else
        ...
        if isRightButton then
            DBGClickLog("cursor over item, calling OnMouseRightCancel()...")
            self.tempObject:OnMouseRightCancel()
        else
    ...
    if isRightButton then
        DBGClickLog("Calling OnMouseRightUp()...")
        self.tempObject:OnMouseRightUp()
    else
...
function GUIInteractionManager:CancelPendingMouseRelease(guiObj)
    ...
    if self.rightClickObj and self.rightClickObj == guiObj then
        self.rightClickObj = nil
    end
...
function GUIInteractionManager:SendKeyEvent(key, down)
    ...
    if key == InputKey.MouseButton0 or key == InputKey.MouseButton1 then
        ...
        if down then
            local result = ProcessMouseClickEvent(self, key)
            return result
        else
            local result = ProcessMouseReleaseEvent(self, key)
            return result
        end
...
function GUIInteractionManager:UpdateMouseCursorPosition()
    ...
    elseif self.rightClickObj then

        DBGCursorHoverLog("Calling OnMouseRightDrag for item '%s'...", Debug_GetBKAForItem(self.rightClickObj:GetRootItem()))
        self.rightClickObj:OnMouseRightDrag()
