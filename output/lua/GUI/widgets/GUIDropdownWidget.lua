...
-- Sets the choice objects visibility, to avoid gui interactions.
local function SetChoiceObjVisibility(self, visible)
    for i = 1, #self.choiceObjs do
        self.choiceObjs[i]:SetVisible(visible)
    end
end
...
function GUIDropdownWidget:_BeginEditing()
    ...
    SetChoiceObjVisibility(self, true)
    self:PerformOpening()
end
...
function GUIDropdownWidget:_EndEditing()
    ...
    SetChoiceObjVisibility(self, false)
    self:PerformClosing()
end
...
local function OnChoicesChanged(self)
    ...
    for i=1, #choices do
        ...
        if not obj then
            ...
            -- If the value == default, put a "*" before it. (text is aligned to the right)
            -- Value should be validated to be not nil.
            local displayString = choices[i].displayString
            if not self.ignoreDefaultAsterisk and choices[i].value == self.default then
...
function GUIDropdownWidget:Initialize(params, errorDepth)
    ...
    RequireType({"boolean", "nil"}, params.ignoreDefaultAsterisk, "params.ignoreDefaultAsterisk", errorDepth)
    ...
    self.ignoreDefaultAsterisk = params.ignoreDefaultAsterisk
