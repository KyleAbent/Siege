...
function GUINumberInputWidget:Initialize(params, errorDepth)
    ...
    self.default = params.default or 0
    
    self:SetValue(self.default) -- initialize to zero.
...
function GUINumberInputWidget:UpdateValueFromUTF8(utf8)
    ...
    -- For some reason, lua has negative zero.... sigh
    if value == -0 then
        value = 0
    end
...
function GUINumberInputWidget:CleanupAndConstrainUTF8(utf8)
    ...
    number = number or self.default -- just in case it's somehow invalid and won't convert to a number.
