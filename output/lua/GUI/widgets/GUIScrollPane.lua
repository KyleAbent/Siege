...
function GUIScrollPane:GetVerticalBar()
    return self.vBar
end

function GUIScrollPane:GetHorizontalBar()
    return self.hBar
end
...
function GUIScrollPane:IsPointOverObject(pt)
    ...
    if self:GetVerticalScrollBarEnabled() and localMousePos.x >= self:GetSize().x - self:GetScrollBarThickness() then
        return false
    end
    
    -- Check if too far down (over horizontal scrollbar).
    if self:GetHorizontalScrollBarEnabled() and localMousePos.y >= self:GetSize().y - self:GetScrollBarThickness() then
        return false
    end
