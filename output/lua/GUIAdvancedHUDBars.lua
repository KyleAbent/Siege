...
--
-- lua/GUIAdvancedHUDBars.lua
--
-- Ported by: Darrell Gentry (darrell@naturalselection2.com)
--
-- Port of the NS2+ HUD Bars. Replaces player health/armor/energy bars with different ones.
...
class 'GUIAdvancedHUDBars' (GUIAnimatedScript)
...
function GUIAdvancedHUDBars:Initialize()
	...
	crosshairScale = ConditionalValue(GetAdvancedOption("crosshairscale") > 1, GetAdvancedOption("crosshairscale"), 1)
	
	local _, _, _, _
	local isMarine = self.team == kTeam1Index
	hudbars = isMarine and GetAdvancedOption("hudbars_m") or GetAdvancedOption("hudbars_a")
...
function GUIAdvancedHUDBars:OnHelpScreenVisChange(state)
...
function GUIAdvancedHUDBars:Uninitialize()
...
function GUIAdvancedHUDBars:Reset()
	...
	if player and player:GetIsAlive() and self.vis then
		...
		if activeWeapon then
			...
			elseif player:isa("Marine") or player:isa("Exo") then
				...
				rightFraction = isReloading and GetReloadFraction() or GetWeaponAmmoFraction(activeWeapon)
				reserveFraction = GetWeaponReserveAmmoFraction(activeWeapon)
		...
		if (player:isa("Marine") or player:isa("Exo") or player:isa("Alien")) and hudbars == 1 then
			...
			if activeWeapon and player:isa("Marine") or player:isa("Exo") then
				local clip = GetWeaponAmmoString(activeWeapon)
				local ammo = GetWeaponReserveAmmoString(activeWeapon)
				local isReloading = activeWeapon:isa("ClipWeapon") and activeWeapon:GetIsReloading()
