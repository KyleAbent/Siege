...
local kDefaultOffset = Vector(100, -430, 0)
local kSpectatorOffset = Vector(450, -325, 0)
local kMarineOffset = Vector(255, -430, 0)
local kAlienOffset = Vector(275, -430, 0)
local kOffset = kDefaultOffset
...
local function UpdateSizeOfUI(self, screenWidth, screenHeight)
    ...
    -- Update all of the current message positions.
    for _, message in ipairs(self.messages) do
        message["Background"]:SetPosition(kOffset * GUIScale(1))
    end

end

local function SetTeamBasedOffset(player)

    kOffset = kDefaultOffset

    if not player or player:GetTeamNumber() == kTeamReadyRoom then

        kOffset = kDefaultOffset

    elseif player:isa("Commander") or player:GetTeamNumber() == kSpectatorIndex then

        kOffset = kSpectatorOffset

    elseif player:GetTeamNumber() == kTeam1Index and GetAdvancedOption("unlocks") then

        kOffset = kMarineOffset

    elseif player:GetTeamNumber() == kTeam2Index and GetAdvancedOption("unlocks")  then

        kOffset = kAlienOffset

    end

end

function GUIChat:UpdatePosition(newPlayer)
    SetTeamBasedOffset(newPlayer)
    UpdateSizeOfUI(self, Client.GetScreenWidth(), Client.GetScreenHeight())
end

function GUIChat:OnLocalPlayerChanged(newPlayer)
    self:UpdatePosition(newPlayer)
end
...
function GUIChat:Initialize()
    ...
    self.inputItem:SetDropShadowEnabled(true)
    self.inputItem:SetLayer(kGUILayerChat)

    self:UpdatePosition(Client.GetLocalPlayer())
...
function GUIChat:AddMessage(playerColor, playerName, messageColor, messageText, isCommander, isRookie)
    ...
    insertMessage["Commander"]:SetDropShadowEnabled(true)
    ...
    insertMessage["Rookie"]:SetDropShadowEnabled(true)
    ...
    insertMessage["Player"]:SetDropShadowEnabled(true)
    ...
    insertMessage["Message"]:SetDropShadowEnabled(true)
    ...
    insertMessage["Message2"]:SetDropShadowEnabled(true)
