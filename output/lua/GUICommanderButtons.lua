...
function GUICommanderButtons:InitializeAlienBackground()
    ...
    self.background:SetTexture(ConditionalValue(self.cachedHudDetail == kHUDMode.Minimal, "ui/transparent.dds", self.backgroundTextureName))
    ...
    self.smokeyBackground:SetTexture(ConditionalValue(self.cachedHudDetail == kHUDMode.Minimal, "ui/transparent.dds", "ui/alien_commander_smkmask.dds"))
...
local function UpdateTabs(self)
    ...
    if not self.selectedTabIndex then
        ButtonPressed(self, self:ConvertTechIdToTabIndex(kCommanderDefaultMenu))
    end
