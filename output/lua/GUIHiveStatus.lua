...
function GUIHiveStatus:Initialize()
    ...
    self.disabled = not GetAdvancedOption("hivestatus")
    self:SetIsVisible(not HelpScreen_GetHelpScreen():GetIsBeingDisplayed() and not self.disabled)
...
function GUIHiveStatus:Update( deltaTime ) 
    ...
    -- Don't update without data source availaible
    if not self.teamInfoEnt then
        return
    end
    ...
    if self.cachedHudDetail ~= newHudDetail then
        ...
        for idx, _ in ipairs(self.statusSlots) do

            if not self.statusSlots[idx]._isEmpty then

                self.statusSlots[idx].frame:SetTexture(ConditionalValue(minimal, "ui/transparent.dds", "ui/alien_hivestatus_frame_bgs.dds"))
                self.statusSlots[idx].locationBackground:SetTexture( ConditionalValue(minimal, "ui/transparent.dds","ui/alien_hivestatus_locationname_bg.dds"))
            end
        end
...
function GUIHiveStatus:CreateStatusContainer( slotIdx, locationId )
    ...
    self.statusSlots[slotIdx].frame:SetTexture( ConditionalValue(minimal, "ui/transparent.dds", kContainerBackgroundTex) )
    ...
    self.statusSlots[slotIdx].locationBackground:SetTexture( ConditionalValue(minimal, "ui/transparent.dds", kLocationBg) )
