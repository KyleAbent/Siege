...
function GUILifeformPopup:UpdateBackgroundYPos()

    local popupOffset = ConditionalValue(GetAdvancedOption("hudbars_a") == 2, kPopupYOffset - 175, kPopupYOffset )
    self.background:SetPosition(Vector(self.background:GetPosition().x, popupOffset * self.scale, 0))
...
function GUILifeformPopup:Uninitialize()
    ...
    PlayerUI_SetLifeformPopupActive(false)
...
function GUILifeformPopup:Update(deltaTime)
    ...
    if state == "hidden" then
        isVisible = self.opacity ~= 0
    elseif state == "presenting" then
        isVisible = self.flyInFraction > 0
    end

    PlayerUI_SetLifeformPopupActive(isVisible)
