...
function GUIMinimapButtons:Initialize()
    ...
    self.background:SetIsVisible(not minimal)
end
...
function GUIMinimapButtons:Update(deltaTime)
    ...
    if self.cachedHudDetail ~= Client.GetHudDetail() then

        self.cachedHudDetail = Client.GetHudDetail()
        self.background:SetIsVisible(self.cachedHudDetail ~= kHUDMode.Minimal)
...
function GUIMinimapButtons:ContainsPoint(pointX, pointY)
    return (self.background:GetIsVisible() and GUIItemContainsPoint(self.pingButton, pointX, pointY)) or
           (self.background:GetIsVisible() and GUIItemContainsPoint(self.techMapButton, pointX, pointY))
end
...
function GUIMinimapButtons:SendKeyEvent(key, down)
    ...
    if key == InputKey.MouseButton0 and self.background:GetIsVisible() and CommanderUI_GetUIClickable() and self.background:GetParent():GetIsVisible() then
