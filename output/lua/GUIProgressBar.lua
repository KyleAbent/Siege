...
function GUIProgressBar:OnLocalPlayerChanged(player)
    self:Uninitialize()
    self:Initialize()
end
...
function GUIProgressBar:Initialize()
    ...
    local inventoryMode = GetAdvancedOption("inventory")
    local inventoryShowingAmmo = self.teamType == kMarineTeamType and (inventoryMode == 2 or inventoryMode == 4)
    ...
    self.progressBarBg:SetPosition(ConditionalValue(inventoryShowingAmmo, GUIProgressBar.kBgPosition - GUIScale(Vector(0, 24, 0)), GUIProgressBar.kBgPosition))
    self.progressBarBg:SetTexture(ConditionalValue(self.cachedHudDetail == kHUDMode.Minimal, "ui/transparent.dds", texture))
...
function GUIProgressBar:InitSmokeBg()
    ...
    self.smokeyBackground:SetTexture(ConditionalValue(self.cachedHudDetail == kHUDMode.Minimal, "ui/transparent.dds", "ui/alien_logout_smkmask.dds"))
...
function GUIProgressBar:InitCircleMask()
    ...
    self.circle:SetTexture(ConditionalValue(self.cachedHudDetail == kHUDMode.Minimal, "ui/transparent.dds", kUnitStatusNeutral))
...
function GUIProgressBar:Update(deltaTime)
    ...
    if self.cachedHudDetail ~= newHudDetail then
        ...
        self.progressBarBg:SetTexture(ConditionalValue(minimal, "ui/transparent.dds", kTextures[self.teamType]))

        if self.circle then
            self.circle:SetTexture(ConditionalValue(minimal, "ui/transparent.dds", kUnitStatusNeutral))
        end

        if self.smokeyBackground then
            self.smokeyBackground:SetTexture(ConditionalValue(minimal, "ui/transparent.dds", "ui/alien_logout_smkmask.dds"))
        end
    end
