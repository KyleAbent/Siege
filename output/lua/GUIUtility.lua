...
local kScreenLinearScaleAspect = 1280 -- Used in GUILinearScale, used in death/endround stats
...
function GUISetUserScale(newScale)

    kUserScale = newScale

    local xRes = Client.GetScreenWidth()
    local yRes = Client.GetScreenHeight()
    GetGUIManager():OnResolutionChanged(xRes, yRes, xRes, yRes) -- Old GUIManager
    GetGlobalEventDispatcher():FireEvent("OnResolutionChanged", xRes, yRes, xRes, yRes) --  New GUIObject
...
function GUIItemContainsPoint(guiItem, pointX, pointY, useAbsScale)
    ...
    local itemScale = 1
    if useAbsScale then
        itemScale = guiItem.GetAbsoluteScale and guiItem:GetAbsoluteScale() or 1
        itemSize = itemSize * itemScale
    else --GUIAnimatedItem's Uniform Scale
...
-- Useful for scaling UI Sizes and positions so they never domainte the screen on small screen aspects.
-- See MenuManager for changing the resolution calculations
function GUIScale(size)
    return (math.min(Client.GetScreenWidth(), Client.GetScreenHeight()) / kScreenScaleAspect * size) * kUserScale
end
...
function GUILinearScale(size)
    ...
    return (ScreenSmallAspect() / kScreenLinearScaleAspect)*size*scale
end
...
function GUIGetSizeFromCoords(coords)
