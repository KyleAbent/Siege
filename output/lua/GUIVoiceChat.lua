...
local function UpdateItemsGUIScale(self)
    ...
    kLifeformPopupOffset = GUIScale(200)

    if PlayerUI_IsOnAlienTeam() then

        kBackgroundOffset.x = kBackgroundOffset.x - GUIScale(70)

        -- Increase offset when player is spectating to account for right side player frames
    elseif PlayerUI_GetIsSpecating() then

        kBackgroundOffset.x = kBackgroundOffset.x - GUIScale(200)
    end
...
function GUIVoiceChat:Update(deltaTime)
    ...
    for i = 1, numPlayers do
        ...
        local voiceChannel, isLocalClient = ChatUI_GetVoiceChannelForClient(clientIndex)
        ...
        if voiceChannel ~= VoiceChannel.Invalid then
            ...
            local chatName = playerName
            -- Notify player that they are muted when they try to talk by
            local isMuted = isLocalClient and (OptionsDialogUI_GetVoiceVolume() <= 0 or Client.GetOptionFloat("recordingGain", 0.5) <= 0)
            if isMuted then
                chatName = string.format("%s (%s)", chatName, Locale.ResolveString("CHATBAR_MUTED"))
            end

            chatBar.Name:SetText(chatName)
            ...
            local newPos = kBackgroundOffset + currentBarPosition
            local lifeFormPopupOffsetActive = PlayerUI_GetLifeformPopupActive()
            newPos.x = newPos.x - ConditionalValue(lifeFormPopupOffsetActive, kLifeformPopupOffset, 0)

            chatBar.Background:SetPosition(newPos)
