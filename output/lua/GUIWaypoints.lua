...
--GUIWaypoints.kMinimal = GetAdvancedOption("minwps")
            ...
            if arrowDist >= 5 then
                ...
                if trace.fraction ~= 1 then
                    ...
                    if distToPlayer > kArrowMinDistToPlayerSquared and distToTarget > kArrowMinDistToTargetSquared then
                        ...
                        arrow.inWorld = true
                        arrow.model:SetIsVisible(Client.kHintsEnabled)
                        arrow.light:SetIsVisible(Client.kHintsEnabled)
...
local function AnimateFinalWaypoint(self)
    ...
    if finalWaypointData then
        ...
        if not self.usedTexture then
            self.finalWaypoint:SetTexture(kTransparentTexture)
            self.usedTexture = kTransparentTexture
        end

        self.finalDistanceText:SetIsVisible(self.visible and fullHUD and Client.kHintsEnabled)
        self.finalNameText:SetIsVisible(self.visible and fullHUD and Client.kHintsEnabled)
