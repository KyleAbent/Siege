...
local function ClearGhostStructure(self)
    ...
    -- Update marine commander bot if there is one
    if self:isa("ScriptActor") then -- I guess this is kinda paranoid

        for _, brain in ipairs(gMarineCommanderBrains) do
            if self.GetLocationName then
                local locationName = self:GetLocationName()
                brain:SetLastPoofTime(locationName)
            end
        end
