...
function DestroyGraphDrivenModel(gdm)
    ...
    if gdm.renderModel then
        gdm.renderModel:ClearOverrideMaterials()    --safety
...
function GraphDrivenModel:Initialize( modelFile, graphFile, optionalRenderSceneZone )   --?? change to use precached?
    ...
    --toggle to force model to the updated regardless of in-frame visibility
...
function GraphDrivenModel:SetMaterialOverride( material, index )
    assert(material and index)
    assert(index >= 0)
    self.renderModel:SetOverrideMaterial( index, material )
end

function GraphDrivenModel:SetMaterialOverrideDelayed( material, index )
    assert(material)
...
function GraphDrivenModel:ClearOverrideMaterials()
    self.renderModel:ClearOverrideMaterials()
end

function GraphDrivenModel:RemoveOverrideMaterial(index)
    assert(index)
    self.renderModel:RemoveOverrideMaterial(index)
end

function GraphDrivenModel:LogAllMaterials()
    self.renderModel:LogAllMaterials()
end

function GraphDrivenModel:SetAlwaysRender(enable)
    assert(enable and type(enable) == "boolean")
    self.alwaysRender = enable
end
...
function GraphDrivenModel:Update()
    ...
    if self.renderModel:GetNumFramesInvisible() > 0 and not self.alwaysRender then
        return
    end
...
function GraphDrivenModel:Destroy()
    ...
    if self.renderModel then
        self.renderModel:ClearOverrideMaterials() --safety call if mat overrides were used
...
local function UpdateGraphDrivenModels()    --FIXME This is going to be effectively call twice for Customize Screen...
