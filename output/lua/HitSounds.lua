if Client then
    ...
    function HitSounds_PlayHitsound( i )
        if kHitSounds[i] then
            StartSoundEffect( kHitSounds[i], kHitSoundVol, kHitSoundPitch )
        end
    end

    function HitSounds_SyncOptions()
        kHitSoundVol = GetOptionsMenu():GetOptionWidget("hitSoundVolume"):GetValue()
        kHitSoundPitch = GetOptionsMenu():GetOptionWidget("hitSoundPitch"):GetValue()
    end
