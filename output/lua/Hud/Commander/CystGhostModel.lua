...
local function UpdateCystModels(self, cystPoints, nbExistingCystUsed, existingCyst)
    ...
    if self.loadedModelIndex then
        ...
        if #cystPoints > 0 then
            ...
            player:AddGhostGuide(Vector(lastcyst.origin), kCystRedeployRange + Cyst.kRedeployBias)
...
function CystGhostModel:Update()
    ...
    if modelCoords then        
        ...
        local numCystPoints = #cystPoints
        
        -- use the last cyst point for the main ghost model
        if numCystPoints > 1 then
            modelCoords = AlignCyst(Coords.GetTranslation(cystPoints[numCystPoints]), normals[numCystPoints])
            self.renderModel:SetCoords(modelCoords)
        end
        
        if self.costDisplay then
        
            local cost = (numCystPoints - nbExistingCystUsed) * kCystCost
        ...
        if not existingCyst[numCystPoints] then
            local redeployCysts = GetEntitiesWithinRange("Cyst", modelCoords.origin, kCystRedeployRange + Cyst.kRedeployBias)
