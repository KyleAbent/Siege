...
function GhostModel:Update()
    ...
    if self.loadedModelIndex ~= modelIndex then
        ...
        -- Is there any reason to EVER have these drop-ables allow decals to be projected on them?
        -- Can't think of a use-case for that... so just setting the highlight to 0.5
        self.renderModel:SetMaterialParameter("highlight", 0.5)
    ...
    if not modelCoords or not modelCoords:GetIsFinite() then
        ...
        --TD-FIXME Need means to set material override per variant of ghost model...
