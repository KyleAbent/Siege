...
function HelpScreen:ArrangeTiles()
    ...
    PROFILE("HelpScreen:ArrangeTiles")
    ...
    if rowCount == 1 then
        ...
        for i=1, #self.tiles do
            self.tiles:GetValueAtIndex(i):SetPosition(start + perTile * (i-1))
        end
    ...
    elseif rowCount == 2 then
        ...
        for i=1, topCount do
            self.tiles:GetValueAtIndex(i):SetPosition(topStart + perTile * (i - 1))
        end
        
        for i=topCount+1, botCount+topCount do
            self.tiles:GetValueAtIndex(i):SetPosition(botStart + perTile * ((i - topCount) - 1))
        end
...
function HelpScreen:MarkAllAsRead()
    ...
    for tileName, __ in pairs(self.tiles) do -- JIT-SAFE pairs! (ordered iterable dict)
        SetTileRead(tileName)
    end
...
local function CreateSingleTile(self, contentTable)
    
    local newScript
    if contentTable.theme == "marine" then
        newScript = GetGUIManager():CreateGUIScript("Hud/HelpScreen/MarineHelpTile")
    elseif contentTable.theme == "alien" then
        newScript = GetGUIManager():CreateGUIScript("Hud/HelpScreen/AlienHelpTile")
    end
    
    assert(newScript)
    
    newScript:SetContent(contentTable)
    newScript:SetIsVisible(self.displayed)
    
    self.tiles[newScript.name] = newScript
...
function HelpScreen:CreateTiles(contentTable)
    ...
    PROFILE("HelpScreen:CreateTiles")
    
    for i=1, math.min(#contentTable, self.kMaxTiles) do
        
        CreateSingleTile(self, contentTable[i])
...
function HelpScreen:DiscardTiles()
    ...
    PROFILE("HelpScreen:DiscardTiles")
    
    if self.tiles then
        for _, tile in pairs(self.tiles) do -- JIT-SAFE pairs (ordered iterable dict)
            GetGUIManager():DestroyGUIScript(tile)
        end
    end
...
function HelpScreen:PositionHotkey()
    ...
    PROFILE("HelpScreen:PositionHotkey")
    ...
    local xOffset = 0
    if teamNumber == kTeam2Type then

        if GetAdvancedOption("hudbars_a") == 2 then
            xOffset = 120 * widthFactor
        end

    else

        if GetAdvancedOption("hudbars_m") == 2 then
            xOffset = 30 * widthFactor
        end

    end

    local x = widthFactor * (hotkeyIconPosition.x - xOffset)
...
function HelpScreen:Initialize()
    ...
    PROFILE("HelpScreen:Initialize")
    
    InitModScripts()

    self.tiles = OrderedIterableDict()
...
function HelpScreen:Uninitialize()
    ...
    PROFILE("HelpScreen:Uninitialize")
...
function HelpScreen:Update(deltaTime)
    ...
    if self.displayed then
        for __, tile in pairs(self.tiles) do -- JIT-SAFE pairs! (ordered iterable dict)
            if tile.hideIfLocked and tile.unlocked == false then
...
function HelpScreen:Hide()
    ...
    for __, tile in pairs(self.tiles) do -- JIT-SAFE pairs
...
local function GetIsApplicableTile(name, applicableTiles)
    for i=1, #applicableTiles do
        if applicableTiles[i].name == name then
            return true
        end
    end
    return false
...
function HelpScreen:RefreshTiles()
    ...
    local applicableTiles = GetAllApplicableTiles()
    
    -- Remove the tiles we no longer need.
    for i=#self.tiles, 1, -1 do
        local tile = self.tiles:GetValueAtIndex(i)
        if not GetIsApplicableTile(tile.name, applicableTiles) then
            -- Destroy this single tile.
            GetGUIManager():DestroyGUIScript(tile)
            self.tiles[tile.name] = nil
        end
    end
    
    -- Add the missing tiles.
    for i=1, #applicableTiles do
        local tile = self.tiles[applicableTiles[i].name]
        if not tile then
            CreateSingleTile(self, applicableTiles[i])
        end
    end
