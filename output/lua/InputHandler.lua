...
local function OnSendKeyEvent(key, down, amount, repeated)
    ...
    if not stop and not Client.GetMouseVisible() then
        ...
        elseif key == InputKey.MouseY then
            ...
            local pitchAmount = amount
            local invertMouse = OptionsDialogUI_GetMouseInverted()
            if invertMouse then
                pitchAmount = -pitchAmount
            end
            
            _cameraPitch = Clamp(_cameraPitch + ApplyMouseAdjustments(pitchAmount, _sensitivityScalarY), -kMaxPitch, kMaxPitch)
...
local function GenerateMove()
    ...
    if not moveInputBlocked then
        ...
        if _keyState[ _keyBinding.ToggleFlashlight ] then
