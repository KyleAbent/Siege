...
function MinimapMappableMixin:UpdateMinimapItem(minimap, item)
    PROFILE("MinimapMappableMixin:UpdateMinimapItem")
...
-- Remove entity mapblip from minimap on destruction
function MinimapMappableMixin:OnDestroy()
    local minimap = ClientUI.GetScript("GUIMinimapFrame") or ClientUI.GetScript("GUIMinimap")
    if minimap then
        local id = self:GetId()
        minimap:RemoveEntityIcon(id)
    end
end
