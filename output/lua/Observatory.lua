...
local networkVars = {
    ...
    lastbeacon = "private time",
    lastAdvancedBeaconTime = "time"
}
...
function Observatory:OnCreate()
    ...
    self.lastAdvancedBeaconTime = nil
    if Server then
...
function Observatory:PerformAdvancedBeacon()
    self.distressBeaconSound:Stop()
    -- Set the timestamp when the beacon completes (for power outage)
    self.lastAdvancedBeaconTime = Shared.GetTime()
...
local function GetRecentlyAdvBeaconed(self)
    -- Only check lastAdvancedBeaconTime, not lastbeacon
    local duration = kObsAdvBeaconPowerOff
    return self.lastAdvancedBeaconTime and (self.lastAdvancedBeaconTime + duration) > Shared.GetTime()
end
