...
function OnCommandHUDMapEnabled(enabled)
    ...
    local widget = GetOptionsMenu():GetOptionWidget("minimap")

    if widget then

        if enabled == nil then

            Print("HUD Minimap (true/false): %s", widget:GetValue() and "Enabled" or "Disabled")

        elseif (enabled == "true" or enabled == "false") then

            local enable = enabled == "true"
            widget:SetValue(enable)
            Print("%s HUD Minimap", enable and "Enabled" or "Disabled")
        end
    else
        Print("Could not find option widget for minimap")
    end
