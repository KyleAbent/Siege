...
-- Returns true if the order type means it should not clear itself as the order queue moves down the line.
function GetIsOrderCyclingType(order)

    local techId = order:GetType()
    return techId == kTechId.Patrol
end

-- Returns true if the order type repeats itself, and only itself, forever.
function GetIsOrderRepeatingType(order)

    local techId = order:GetType()
    return techId == kTechId.Follow or techId == kTechId.FollowAndWeld
end

-- Returns true if the order should not clear itself after processing/completion.
function GetIsOrderLoopingType(order)
    return (GetIsOrderRepeatingType(order) or GetIsOrderCyclingType(order))
end
...
function Order:OnCreate()
    ...
    self.orderParam = Entity.invalidId
...
kOrderTypesUseEntityOrigin[kTechId.Attack] = true
...
function Order:GetOrderSource()
    ...
    if not owner or self:GetType() == kTechId.Patrol then -- Other looping types such as Follow don't use the original order origin.
