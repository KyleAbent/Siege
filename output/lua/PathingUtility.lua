...
function ParsePathingSettings(settings)
    ...
    SetPathingOption(Pathing.Option_AgentRadius, 0.75)
...
function GeneratePath(src, dst, doSmooth, smoothDist, maxSplitPoints, allowFlying)
    ...
    if gDebugPathing then
        ...
        if points then
            local numPoints = #points
            for i = 1, numPoints do
                DebugCapsule(points[i], points[i], 0.2, 0.2, 2)
            end
        end
...
function GetPointDistanceSquared(points)
    if (points == nil) then
        return 0
    end
    local numPoints   = #points
    ...
    while i <= numPoints do -- bugfix here < -> <=
        if (i > 1) then
            distance = distance + (points[i - 1] - points[i]):GetLengthSquared()
        end
        i = i + 1
