...
function Player:SetControllerClient(client, isPickup)
    ...
    if client ~= nil then
        ...
        if self.isVirtual and self.client.bot and self.client.bot.brain then
            self.client.bot.brain:ClearAllVisitedLocations()
        end
...
function Player:ProcessBuyAction(techIds)
    ...
    if totalCost <= self:GetResources() then
        ...
        if not self:GetIsVirtual() then
            --Print("not enough resources sound server")    --McG: why was this here?
