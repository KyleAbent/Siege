...
function PlayingTeam:ResetTeam()
    ...
    if self.GetTeamBrain then
        self:GetTeamBrain():SetInitialTechPoint(initialTechPoint)
    end
...
function PlayingTeam:Reset()
    ...
    if self.brain ~= nil then
        Log("Reset AI TeamBrain for %s", (self.teamNumber == kTeam1Index and "Marines" or "Aliens"))
        self.brain:Reset()
    end
...
function PlayingTeam:TriggerAlert(techId, entity, force)
    ...
    if GetGamerules():GetGameStarted() then
        ...
        if self.brain ~= nil then
            self.brain:AddNewAlert( techId, entity:GetId(), entity:GetOrigin() )
        end
...
--leave as is, this is intended, as actual Team classes should override this
    return nil
end
...
function PlayingTeam:UpdateVotes()
    ...
    if self.ejectCommVoteManager:GetVotePassed() then
        ...
        local userId = self.ejectCommVoteManager:GetTarget()
        local targetCommander

        if userId == 0 then
            local commBot = GetGamerules().botTeamController:GetCommanderBot(self:GetTeamNumber())
            targetCommander = commBot and commBot:GetPlayer()
        else
            targetCommander = GetPlayerFromUserId(self.ejectCommVoteManager:GetTarget())
        end
