...
function Tunnel:OnCreate()
    ...
    if Server then
        ...
        --Build interior path-points for AI units
        --self:GenerateInteriorPathingPoints()
...
if Server then
    ...
    function Tunnel:GetIsConnected()
        return (self.exitAConnected and self.exitBConnected)
    end

    -- If one exit has been destroyed, returns the remaining exit point
    function Tunnel:GetEmergencyExitOrigin()
        if self.exitAConnected then
            return self:GetExitAPosition()
        elseif self.exitBConnected then
            return self:GetExitBPosition()
        end
    ...
    local function ApplyDOTToEntity(self, ent, dps, deltaTime)
        ...
        if ent:isa("Exo") then
            ent:TakeDamage(dps * deltaTime, self, nil, nil, nil, dps * deltaTime, 0, kDamageType.Normal)
        else
            ent:TakeDamage(dps * deltaTime, self, nil, nil, nil, 0, dps * deltaTime, kDamageType.Normal)
        end
    ...
    function Tunnel:UpdateCollapseDoT()
        ...
        for i = entCount, 1, -1 do
            ApplyDOTToEntity(self, ents[i], kTunnelCollapseDPS, deltaTime)
        end
    ...
    function Tunnel:MovePlayerToTunnel(player, entrance)
        ...
        player.currentTunnelId = self:GetId()
...
if Server then
    ...
    --[[
    local kDebugDrawTunnelPathing = false
    Event.Hook("Console_sv_dbg_tunnel_path", function(client)
        ...
        --find tunnel to draw pathing
        local tunnel = GetIsPointInGorgeTunnel(player:GetOrigin())

        if tunnel then
            local path = tunnel.interiorPathPoints

            for i = 1, #path do
                DebugPoint( path[i], 1, 600 )

                if i > 1 then
                    DebugLine( path[i - 1], path[i], 600, 0, 0.2, 1, 0.6 )
                end
            end
        end
