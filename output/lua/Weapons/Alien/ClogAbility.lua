...
function ClogAbility:GetGhostModelName(ability)
    ...
    if player and player:isa("Gorge") then
        ...
        local variant = clientVariants.clogVariant
        
        if variant == kClogVariants.Shadow then
            return Clog.kModelNameShadow
        elseif variant == kClogVariants.Toxin then
            return Clog.kModelNameToxin
        elseif variant == kClogVariants.Abyss then
            return Clog.kModelNameAbyss
        elseif variant == kClogVariants.Kodiak then
            return Clog.kModelNameKodiak
        elseif variant == kClogVariants.Reaper then
            return Clog.kModelNameReaper
        elseif variant == kClogVariants.Auric then
            return Clog.kModelNameShadow
        elseif variant == kClogVariants.Nocturne then
            return Clog.kModelNameNocturne
        end
