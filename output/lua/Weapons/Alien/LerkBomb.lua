...
if Server then
    ...
    function LerkBomb:ProcessHit(targetHit, surface, normal, endPoint)
        if targetHit and GetAreEnemies(self, targetHit) then
            -- Detonate immediately when hitting enemies
            self:Detonate(targetHit)
        else
            -- When hitting environment, start countdown but don't explode
            if Server then
                -- Set a new countdown (4 seconds)
                self:AddTimedCallback(LerkBomb.TimeUp, 4)

                -- Optional: Add a visual effect showing it's live
                self:TriggerEffects("lerkbomb_armed")
            end
        end
    end
