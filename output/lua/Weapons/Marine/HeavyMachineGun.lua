...
function HeavyMachineGun:GetPrimaryAttackPlaysClientSound()
    return true
end
...
function HeavyMachineGun:OnUpdateAnimationInput(modelMixin)
    ...
    local reloadMultiplier = HeavyMachineGun.kBaseReloadMultipier
...
if Client then
    ...
    function HeavyMachineGun:OnFilteredCinematicOptionChanged()
        DestroyMuzzleEffect(self)
        DestroyShellEffect(self)
    end
    ...
    function HeavyMachineGun:GetUIDisplaySettings()
        return { xSize = 256, ySize = 160, script = "lua/GUIHeavyMachineGunDisplay.lua", variant = self:GetHMGVariant() }
    end
