...
kPoofRetryDelay = 10 -- How often to try to re-drop a structure that has been poofed (any structure, in a location with same name)
...
--]]
    ...
    function(bot, brain, com)
        ...
        local kMaxForwardObservatories = 1 -- Not including main base observatory
    ...
    function(bot, brain, com)
        ...
        if doables[kTechId.DropJetpack] and #prototypeLabs > 0 and not isZeroIPs and not brain:GetIsProcessingTechPathOverride() then
            ...
            local maxJetpacks = #senses:Get("nonJPMarines")
    ...
    function(bot, brain, com)
        ...
        for i, alert in ipairs(alertqueue) do
            ...
            local target
            if targetTechId and time - alert.time < kAlertLifetime and doables[targetTechId] then
    ...
    function(bot, brain, com)
        ...
        if structures and #macs < kMaxMacs and not brain:GetIsProcessingTechPathOverride() then
