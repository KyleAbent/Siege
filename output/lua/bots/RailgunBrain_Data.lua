-- BOT-TODO: file unused, coalesced into ExoBrain_Data.lua
...
local function GetCanAttack(marine)
    ...
    if weaponHolder ~= nil then 
        ...
        if leftWeapon:isa("Railgun") and rightWeapon:isa("Railgun") then
            return true
        else
            return false
        end
...
kRailgunBrainActions =
{
    ...
    function(bot, brain)        --FIXME Refactor for Exo to allow for self-welding
    ...
    ------------------------------------------
    --[[
        McG: Exos should not explore, they're assault tanks...

    CreateExploreAction( 0.05, 
...
function CreateRailgunBrainsSenses()
