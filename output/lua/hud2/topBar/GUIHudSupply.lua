...
function GUIHudSupply:UpdateSupplyHook()
    ...
    local eventName = string.format("OnTeam%dSupplyUsedChanged", self:GetTeamNumber())
    
    self.supplyHook = self:HookEvent(GetGlobalEventDispatcher(), eventName, self.SetSupply)
