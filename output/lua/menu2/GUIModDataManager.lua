...
GUIModDataManager:AddClassProperty("ModIndexes", {})
...
function GUIModDataManager:OnUpdate(deltaTime, now)
    ...
    for i = 1, Client.GetNumMods() do
        if not Client.GetIsModInternal(i) then
            modCount = modCount + 1
            modIndexes[modCount] = i
        end
    end

    self:SetModIndexes(modIndexes)
    self:SetNumMods(modCount)
