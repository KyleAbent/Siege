...
function MenuBackgrounds.GetMenuBackgroundChoices()
    ...
    table.insert(choices, 1, { value = "default", displayString = Locale.ResolveString("MENU_BACKGROUND_DEFAULT") })
    table.insert(choices, 2, { value = "random", displayString = Locale.ResolveString("MENU_BACKGROUND_RANDOM") })
...
--In order to prevent "old" background (when default selected) from loading on NEW update (map release)
--we must read the NAME from options first, then compare stored default path with defined default path.
function MenuBackgrounds.GetCurrentMenuBackgroundCinematicPath()
    ...
    if backgroundName and backgroundName == "default" then
        if cinematicPath ~= kDefaultMenuBackgroundPath then
            cinematicPath = kDefaultMenuBackgroundPath
        end
    end
...
{
    name = "tanith",
    localeString = "MENU_BACKGROUND_TANITH",
    cinematicPath = "cinematics/menus/main_menu_tanith.cinematic",
})
...
{
    name = "metro",
    localeString = "MENU_BACKGROUND_METRO",
    cinematicPath = "cinematics/menus/main_menu_metro.cinematic",
})


MenuBackgrounds.SetDefaultBackground("tanith")
