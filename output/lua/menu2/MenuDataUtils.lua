...
function MenuData.SetMouseSensitivity(sens)
    Client.SetMouseSensitivity(sens)
    Client.SetOptionFloat("input/mouse/sensitivity", sens)
end
...
-- paramsTable should contain two keys: checksChildren, and regularChildren, each is a table array
-- containing children to place into these sections.
function MenuData.CreateExtendedOptionsLayout(paramsTable)
    return
    {
        name = "scrollPane",
        class = GUIMenuScrollPane,
            ...
            MenuData.CreateVerticalListLayout
            {
                children =
                {
                    ...
                    { -- Extended Section
                        ...
                        children = paramsTable.extendedChildren,
                    },
                },
            },
        },
...
function MenuData.GetMenuMusicChoices()
    ...
    table.insert(choices, 1, { value = "random", displayString = Locale.ResolveString("MENU_MUSIC_RANDOM") })
...
function MenuData.GetResolution()
    ...
    local xRes = Client.GetOptionInteger("graphics/display/x-resolution", startupRes.xResolution)
    local yRes = Client.GetOptionInteger("graphics/display/y-resolution", startupRes.yResolution)
    local resolutions = GetResolutionsTable()
...
function MenuData.PreviewHitSound()
    ...
    local pitch = GetOptionsMenu():GetOptionWidget("hitSoundPitch"):GetValue()

    local before = Client.GetOptionFloat("hitsound-vol", 0)
    local beforePitch = Client.GetOptionFloat("hitsound-pitch", 0)

    Client.SetOptionFloat("hitsound-vol", volume)
    Client.SetOptionFloat("hitsound-pitch", pitch)

    HitSounds_SyncOptions()
    HitSounds_PlayHitsound(1)

    Client.SetOptionFloat("hitsound-vol", before)
    Client.SetOptionFloat("hitsound-pitch", beforePitch)

    HitSounds_SyncOptions()

end

local installedMapList = {}

function MenuData.GetMapChoices()
    
    Client.RefreshModList()
    
    local mapFileNames = GetInstalledMapList()
    installedMapList = mapFileNames
...
function MenuData.SetListenServerMap(optionPath, index)

    local res = installedMapList[index]

    if not res then
        Log("Bad listen server map index %s max is %s", index, #installedMapList)
        return
    end

    Log("SetListenServerMap: map %s, mod %s", res.fileName, res.modId)

    Client.SetOptionString(kStartServer_MapKey, res.fileName)
    Client.SetOptionInteger64(kStartServer_MapMod, res.modId or 0)
end

function MenuData.GetListenServerMap()

    if #installedMapList == 0 then
        MenuData.GetMapChoices()
    end

    local map = Client.GetOptionString(kStartServer_MapKey, "maps/ns2_summit.level")
    local mapMod = Client.GetOptionInteger64(kStartServer_MapMod, 0)

    for i=1, #installedMapList do
        if installedMapList[i].fileName == map and (mapMod == 0 or installedMapList[i].modId == mapMod) then
            return i
        end
    end
    return 1
...
local function StartSinglePlayerMode(localModPath, mapName)
    local modId = Client.GetLocalModId(localModPath)
    assert(modId ~= 0)
    ...
    if Client.StartServer({modId}, mapName, serverName, password, port, maxPlayers, true, true) then
        LeaveMenu()
    end
end
