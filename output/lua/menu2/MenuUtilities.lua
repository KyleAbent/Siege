...
function Client.GetOptionColor(name, defaultValue)
    ...
    return (ColorIntToColor(Client.GetOptionInteger(name, defaultValue))) -- Since this calls GetOptionInteger, it already does translation.
end
...
-- Sets the value of an option-wrapped widget to the passed in value.
function SetWidgetValueToDefault(widget)

    local optionType = widget.optionType
    local default = widget.default

    AssertIsaGUIObject(widget)
    assert(type(default) ~= "nil", "Use of reset button requires a default value!")

    local processedValue = default
    if optionType == "color" then
        processedValue = ColorIntToColor(default)
    end

    widget:SetValue(processedValue)
...
function SetWidgetValueFromOption(widget)
    ...
    if widget.GetMaxValue and widget.GetMinValue then -- To prevent people from read-only exploit
        value = Clamp(value, widget:GetMinValue(), widget:GetMaxValue())
    end
