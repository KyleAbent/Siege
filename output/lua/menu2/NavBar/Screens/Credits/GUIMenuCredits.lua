...
--[[
    These have to have enough of a positive difference such that that difference of time provides enough leeway for the update rate of the
    credits screen will be able to transition the opacity of the image low enough to be set visible/invisible.

    The alpha step of the crossfade is currently set to 0.01, and the update rate is ~30 times a second. The max opacity is kBackgroundAlphaInit (0.32),
    so the difference should have a minimum of at least 1.03 seconds...
]]
local kBgImageScreenTime = 10
local kBgImageFadeTime = 5

local kBackgroundAlphaInit = 0.32 -- Max opaqueness of a background image.
...
function GUIMenuCredits:Display(immediate)
    ...
    self.activeImageScreenTime = ConditionalValue(self.activeImageScreenTime == 0, Shared.GetTime(), self.activeImageScreenTime + (Shared.GetTime() - self.activeImageScreenTime))
...
function GUIMenuCredits:OnUpdate(delta, time)
    ...
    --Update background image rotation and transitions
    ...
    if time >= fadeStartTime and time < fadeEndTime then
        ...
        if newCurColor.a < alphaStep and self.backgroundImages[self.activeBackgroundIndex]:GetVisible() then
            self.backgroundImages[self.activeBackgroundIndex]:SetVisible(false)
        end

    end

    if time >= fadeEndTime then
