...
GUIMenuOptions.SubMenuType = enum({'UseHeaderButton', 'Other'})

local kSubMenuInterfaceData =
{
    __tostring = function() return "{...}" end,
    
    { -- 1 - General
        menuType = GUIMenuOptions.SubMenuType.UseHeaderButton,
        name = "general",
    ...
    { -- 2 - Controls
        menuType = GUIMenuOptions.SubMenuType.Other,
        name = "controls",
    ...
    { -- 2 - Advanced
        menuType = GUIMenuOptions.SubMenuType.UseHeaderButton,
        name = "advanced",
        subScreenName = "Advanced",
        localeString = "MENU_ADVANCED",
        polygon = -- relative to "buttonBar" locator item.
        {
            Vector(552,   0, 0),
            Vector(552, 118, 0),
...
function GUIMenuOptions:GetSubMenuInterfaceData()
    return kSubMenuInterfaceData
end
...
local function FindButtonDataContainingValueForKey(self, key, value)
    ...
    local subMenuInterfaceData = self:GetSubMenuInterfaceData()
    for i=1, #subMenuInterfaceData do
        if subMenuInterfaceData[i][key] == value then
            return subMenuInterfaceData[i]
        end
    end
...
local function ActivateSubScreen(self, buttonData, instant)
    ...
    local subScreenName = buttonData.subScreenName
    ...
    SetGlowingButtonByName(self, buttonName)

    self.activeSubScreenName = subScreenName
    
    UpdateBottomButtons(self)
    
    -- If the mods menu hasn't been refreshed yet, do it now.
    if subScreenName == "Mods" and not self.modsMenuFirstRefresh then
...
function GUIMenuOptions:SetSubScreenActive(name, instant)
    ...
    local screenInterfaceData = FindButtonDataContainingValueForKey(self, "subScreenName", name)
    assert(screenInterfaceData)
    ActivateSubScreen(self, screenInterfaceData, instant)
...
local function InitAdvancedOptionsMenu(self)

    assert(self.advancedCategoryDisplayBox)
    assert(self.advancedCategoryDisplayBox:isa("GUIMenuCategoryDisplayBox"))

    assert(type(gAdvancedSettingsCategories) == "table")

    for i=1, #gAdvancedSettingsCategories do

        local category = gAdvancedSettingsCategories[i]
        self.advancedCategoryDisplayBox:AddCategory(category.categoryName, category.entryConfig, category.contentsConfig)
...
function GUIMenuOptions:Initialize(params, errorDepth)
    ...
    local counter = 1 -- Since we can have non-header-button submenus, its possible to leave an index empty.
    for i=1, #subMenuInterfaceData do

        if subMenuInterfaceData[i].menuType == GUIMenuOptions.SubMenuType.UseHeaderButton then

            self.buttons[counter] = subMenuInterfaceData[i].name
            self.buttons[subMenuInterfaceData[i].name] = CreateHeaderButton(self, subMenuInterfaceData[i])
            counter = counter + 1
    ...
    MenuData.AddDebugInfo(MenuData.Config.OptionsMenu.Advanced, "MenuData.Config.OptionsMenu.Advanced")
    ...
    self.advancedMenuHolder  = CreateSubMenu(self, "advancedMenu", MenuData.Config.OptionsMenu.Advanced)
    ...
    for i=1, #self.subMenuHolders do
        local ha = subMenuInterfaceData[i].xAnchor
...
function GUIMenuOptions:RevertChanges()
    ...
    if self.previousValues then
        for widget, prevValue in pairs(self.previousValues) do -- JIT-SAFE pairs.
            ...
            SetOptionFromWidgetValue(widget) -- Save to file
            reloadGraphicsOptions = (widget.revertGraphicsOptions and true or false)
        end
    end
    if self.previousSubValues then
        for widget, subOptions in pairs(self.previousSubValues) do -- JIT-SAFE pairs.
...
local function GetRecentlyAcceptedLobbyInvitation()
    if Thunderdome():GetHasRecentlyJoinedInvitedLobby() then
        ...
        buttonConfig =
        {
            -- Save changes button
            {
                ...
                callback = function(self)
                    PendingChangesPopupCallback_Save(self)
                    GetRecentlyAcceptedLobbyInvitation()
                end
            },
            ...
            -- Revert changes button
            {
                ...
                callback = function(self)
                    PendingChangesPopupCallback_Revert(self)
                    GetRecentlyAcceptedLobbyInvitation()
                end,
            },
...
function GUIMenuOptions:Display(immediate)

    --Block clients from potentially triggering a main vm restart
    if not Thunderdome():GetIsIdle() then
        self:GetOptionWidget("menuBackground"):SetVisible(false)
        self:GetOptionWidget("language"):SetVisible(false)
        self:GetOptionWidget("menuMusic"):SetVisible(false)
        
        self.buttons.mods:SetEnabled(false)
        
        if self.activeSubScreenName == "Mods" then
            self:SetSubScreenActive("General", true)
        end
    else
        self:GetOptionWidget("menuBackground"):SetVisible(true)
        self:GetOptionWidget("language"):SetVisible(true)
        self:GetOptionWidget("menuMusic"):SetVisible(true)

        self.buttons.mods:SetEnabled(true)
    end
