...
function HostGame(mapName, hidden, serverName, password, maxPlayers, port, disableMods, mods)
    ...
    if mods then
        Client.StartServer(mods, mapName, serverName, password, port, maxPlayers, disableMods == true, hidden == true)
    else
...
function(address, password)
    ...
    if Shared.GetThunderdomeEnabled() then
        Log("Cannot join from Steam while in Matched Play servers")
        return
    end
    
    if Thunderdome():GetIsConnectedToLobby() then
        GetMainMenu():DisplayPopupMessage( 
            Locale.ResolveString("THUNDERDOME_MATCHMAKING_DISALLOW_STEAMJOIN_MSG"), 
            Locale.ResolveString("THUNDERDOME_MATCHMAKING_DISALLOW_STEAMJOIN_TITLE") 
        )
        return
    end
...
function(address, password)
    ...
    if Shared.GetThunderdomeEnabled() then
        Log("Connecting to servers is disabled in Matched Play servers")
        return
    end

    if Thunderdome():GetIsConnectedToLobby() then
        Log("Cannot connect to server while in a Matched Play lobby")
        return
    end
...
function(mapName, hidden, mod)

    if Shared.GetThunderdomeEnabled() then
        Log("Changing map to localhost is disabled in Matched Play servers")
        return
    end

    if Thunderdome():GetIsConnectedToLobby() then
        Log("Cannot load a map while in a Matched Play lobby")
        return
    end
...
local function OnRetryCommand()
    ...
    if Shared.GetThunderdomeEnabled() then
        Log("This command is disabled in Matched Play servers")
        return
    end

    if Thunderdome():GetIsConnectedToLobby() then
        Log("Cannot connect to a server while in a Matched Play lobby")
        return
    end
...
Event.Hook("GameInviteAccepted", function(connectString, friendSteamId64)
    ...
    if Shared.GetThunderdomeEnabled() then
        Log("Error: Cannot accept game-invites while in Matched Play servers")
        return
    end
...
local mapNameSort = function(a, b)
    local result = SB_CompareStrings(a:GetMapName(), b:GetMapName())
    return result
end
...
ServerBrowserSortFunctions.MapName = CombineSorts(mapNameSort, quickPlayRankSort)
ServerBrowserSortFunctions.MapNameReversed = CombineSorts(CreateReversedSort(mapNameSort), quickPlayRankSort)
