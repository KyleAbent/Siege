...
function GUIMenuStartServerScreen:OnStartServerPressed()
    ...
    if Client.GetOptionBoolean(kStartServer_LoadHotfixMods, true) then
        local hotfixMod = ModServices.GetHotfixListModId()
        local hotfixes = hotfixMod and ModServices.GetModDependencies(hotfixMod)

        if hotfixes and #hotfixes > 0 then
            mods = mods or {}

            for _, v in pairs(hotfixes) do
                mods[#mods + 1] = v
            end
        end
    end
