...
-- Refresh friends list every 5 seconds. (This only happens when the menu is open to the player
-- profile screen, so performance isn't as much of a concern here.)
local kFriendsListRefreshInterval = 5
...
function GUIMenuFriendsList:RefreshFriendsList()
    
    local playerScreen = GetPlayerScreen()
    local now = Shared.GetTime()
    
    if (playerScreen and
       playerScreen:GetScreenDisplayed() and
       MainMenu_GetIsOpened() and
       now >= nextAllowedFriendsListUpdateTriggering) and
       self:GetVisible() then
...
local function OnSteamFriendsUpdated(self, friendsTbl)
    ...
    for i=1, #friendsTbl do
        ...
        if self.disableJoinOrInvites == true then
        --forcibly hide join/invite button while in TD lobby
            entryObj:SetButtonsVisbility(false)
        end
...
function GUIMenuFriendsList:Initialize(params, errorDepth)
    ...
    --simple flag to denote if Join/Invite buttons should be visible/enabled
    self.disableJoinOrInvites = false
    ...
    self:HookEvent(GetPlayerScreen(), "OnScreenDisplayedChanged", self.RefreshFriendsList)
    
end

function GUIMenuFriendsList:Display(immediate)

    self.disableJoinOrInvites = not Thunderdome():GetIsIdle()

    --Run immediately, so cannot cheese join/invites
    for steamId64, entry in pairs(self.friendEntries) do
        if self.disableJoinOrInvites then
            entry:SetButtonsVisbility(false)
        else
            entry:ClearButtonsVisibilityOverride()
        end
    end
