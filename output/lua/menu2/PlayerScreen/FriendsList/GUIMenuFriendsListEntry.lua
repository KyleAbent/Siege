...
local function UpdateInviteFriendButtonVisibility(self)
    ...
    if not Client.GetIsConnected() or GetIsOnSameServerTogether(self) or not Thunderdome():GetIsIdle() then
        self.inviteFriendButton:SetVisible(false)
        return
    end
...
local function UpdateJoinFriendButtonVisibility(self)
    ...
    if GetIsOnSameServerTogether(self) or not Thunderdome():GetIsIdle() then
        self.joinFriendButton:SetVisible(false)
        return
    end
...
function GUIMenuFriendsListEntry:Initialize(params, errorDepth)
    ...
    PROFILE("GUIMenuFriendsListEntry:Initialize")
...
function GUIMenuFriendsListEntry:SetButtonsVisbility(visible)
--Override type utility to allow for immediate changes when Player Screen made visible
    self.joinFriendButton:SetVisible(visible)
    self.inviteFriendButton:SetVisible(visible)
end

function GUIMenuFriendsListEntry:ClearButtonsVisibilityOverride()
--force "normal" state to be refreshed
    UpdateJoinFriendButtonVisibility(self)
    UpdateInviteFriendButtonVisibility(self)
end
