...
local function UpdateResetButtonFXState(self)
    ...
    self.resetButton:SetEnabled(ColorIntToColor(self.default) ~= self:GetValue())

end

local function UpdateLabelConstrainedArea(self)
    local maxLength = self:GetSize().x - (self.valueDisplay:GetSize().x + (kSpacing*3))
    local labelTextSize = self.label:GetTextSize()
    local labelWidth = math.min(labelTextSize.x, maxLength)
    self.label:SetSize(labelWidth, labelTextSize.y)
end

local function UpdateHeaderLayout(self)
    self.valueDisplay:SetWidth(self.valueDisplay:GetSize().y * 4)
end
...
function GUIMenuColorPickerWidget:Initialize(params, errorDepth)
    ...
    if params.useResetButton and params.default ~= nil then
        ...
        self.resetButton = CreateGUIObject("resetButton", GUIResetButton, self.header, { noAutoConnectToParent = true })
        self.resetButton:SetPosition(elementX, 0)
        self.resetButton:AlignLeft()
        self.resetButton:SetLayer(self.header:GetLayer() + 1)
