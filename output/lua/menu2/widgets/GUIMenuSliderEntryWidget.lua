...
local function UpdateLayout(self)
    ...
    local currentX = MenuStyle.kWidgetPadding
    if self.resetButton then

        self.resetButton:SetPosition(currentX, 0)
        currentX = currentX + self.resetButton:GetSize().x + MenuStyle.kWidgetPadding

    end

    self.leftSideHolder:SetPosition(currentX, 0)

    local leftSideHolderChildrenX = 0
    self.label:SetPosition(leftSideHolderChildrenX, 0)
    ...
    leftSideHolderChildrenX = leftSideHolderChildrenX + self.label:GetPosition().x + labelNewWidth
    leftSideHolderChildrenX = leftSideHolderChildrenX + MenuStyle.kLabelSpacing
    
    -- Position the entry.
    self.entry:SetPosition(leftSideHolderChildrenX, 0)

    currentX = currentX + leftSideHolderChildrenX
...
function GUIMenuSliderEntryWidget:Initialize(params, errorDepth)
    ...
    RequireType({"boolean", "nil"}, params.useResetButton, "params.useResetButton", errorDepth)
...
function GUIMenuSliderEntryWidget:LoseFocus()
    self.entry:OnEnterKey()
end
