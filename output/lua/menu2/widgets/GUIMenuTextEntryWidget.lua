...
Script.Load("lua/GUI/widgets/GUIResetButton.lua")
...
local function UpdateEntryHolderSize(self)
    ...
    self.entryHolder:SetPosition(self.label:GetPosition().x + self.label:GetSize().x, self.entryHolder:GetPosition().y)
...
-- ugh
local function SetLabelHighlightColor(self, color)
    self.label:GetObject().highlightColor = color or MenuStyle.kHighlight
end

local function SetLabelEditingColor(self, color)
    self.label:GetObject().editingColor = color or MenuStyle.kHighlight
end
...
do -- Define extended methods for the two classes.
    ...
    local function GetInitializeBody(cls, compositeClass, defaultType, includeNumeric)
        ...
        return function(self, params, errorDepth)
            ...
            RequireType({"string", "nil"},    params.labelFont,     "params.labelFont", errorDepth)
            RequireType({"number", "nil"},    params.labelFontSize, "params.labelFontSize", errorDepth)

            RequireType({"string", "nil"},    params.entryFont,     "params.entryFont", errorDepth)
            RequireType({"number", "nil"},    params.entryFontSize, "params.entryFontSize", errorDepth)
            RequireType({"number", "nil"},    params.entryYOffset, "params.entryYOffset", errorDepth)
            ...
            local labelFont =
            {
                family =  params.labelFont or MenuStyle.kOptionFont.family,
                size =  params.labelFontSize or MenuStyle.kOptionFont.size,
            }
            ...
            self.entryHolder:SetAutoScroll(true)
            self.entryHolder:AlignLeft()
            self.entryHolder:SetPosition(-elementX, params.entryYOffset or 0)

            local entryFont =
            {
                family =  params.entryFont or MenuStyle.kOptionFont.family,
                size =  params.entryFontSize or MenuStyle.kOptionFont.size,
            }
            ...
            self:HookEvent(self.entry, "OnCharacterAccepted", UpdateResetButtonFXState)
            self:HookEvent(self.entry, "OnCharacterDeleted", UpdateResetButtonFXState)
    ...
    GUIMenuTextEntryWidget.SetLabelEditingColor = SetLabelEditingColor
    GUIMenuNumberEntryWidget.SetLabelEditingColor = SetLabelEditingColor

    GUIMenuTextEntryWidget.SetLabelHighlightColor = SetLabelHighlightColor
    GUIMenuNumberEntryWidget.SetLabelHighlightColor = SetLabelHighlightColor
