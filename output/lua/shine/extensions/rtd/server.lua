/*
2025 - Migrated from Mar. 2017.
Known Issues: CreateTimer errors when Player Dies/Changes Class/ETC
    Old Reference to Non Existant Player
    Model Size not implemented, etc.
*/
...
function Plugin:GetRTDManager()
    return Shared.GetEntitiesWithClassname("RTDManager")
end

function Plugin:RollPlayer(Player)

  local manager = self:GetRTDManager()
...
--                       return
    ...
    if Player:GetIsAlive() and Player:GetTeamNumber() == 2 and not Player:isa("Commander") then
        ...
        if AlienRoll == 1 then
            ...
            //self:NotifyAlien( nil, "%s roll number 2. Calculating how much res the player has.", true, Player:GetName())
