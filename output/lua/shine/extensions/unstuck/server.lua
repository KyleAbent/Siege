...
function Plugin:ShouldPlayerBeUnstuck( Player )
	...
	if not GetSetupConcluded() then
        ...
        local frontdoor = GetNearest(Player:GetOrigin(), "FrontDoor", 0, function(ent) return Player:GetDistance(ent) <= 14   end)//Frontdoor parent is Siegedoor, frontdoor isa siegedoor lol
        if frontdoor then
            self:NotifyTranslatedError( Player:GetClient(), "Setup time has further radius by front door which you cant unstuck" )
            return false
        end
	
	end
	
	local frontdoor = GetNearest(Player:GetOrigin(), "SiegeDoor", 0, function(ent) return Player:GetDistance(ent) <= 7   end)//Frontdoor parent is Siegedoor, frontdoor isa siegedoor lol
	if frontdoor then
	    self:NotifyTranslatedError( Player:GetClient(), "Cannot unstuck by door" )
	    return false
	end
